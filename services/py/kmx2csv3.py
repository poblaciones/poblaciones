...
def main():
    ...
    try:
        ...
        folder_index = sys.argv[5]
        converter = Converter(in_file, out_folder)

        if generate_files == True:
          out_file = in_file + '_out.csv'
          if file_extension == 'kml':
            files_list = converter.process_kml(in_file, out_file, folder_index)
          elif file_extension == 'kmz':
            files_list = converter.process_kmz(out_file, folder_index)
        else:
          files_list = []
          if file_extension == 'kml':
...
class Converter:
  ...
  def process_kmz(self, out_file, folder_index):
    files = self.extract_kmz();
    for file in files:
       self.process_kml(file, out_file, folder_index)
    return
  ...
  def process_kml(self, file_kml, out_file, folder_index):
    ...
    self.parseExtraColumns(placemarks);
  ...
  def parseExtraColumns(self, placemarks):
    extraColumns = []
    for placemark in placemarks:
      if len(placemark.get_places()) > 0:
        ...
        folder_name = placemark.get_folder_name()
        if not folder_name is None and folder_name != '':
          self.hasFolderName = True
  ...
  def createTitle(self):
    ...
    if self.hasName:
      title.append('Nombre')
    if self.hasFolderName:
      title.append('Carpeta')
  ...
  def createRow(self, placemark, place):
      ...
      if self.hasFolderName:
        row.append(self.formatLine(placemark.get_folder_name()))
...
class Document:
    ...
    def get_all_placemarks(self):
        return Folder(self.xml).get_placemarks()

    def get_folder_placemarks_by_index(self, folder_index):
      n = 1
      for folder in self.get_folders():
        if int(folder_index) == n:
          return folder.get_placemarks()
        n += 1
...
class Placemark:
    ...
    def __parse__(self, xml):
        ...
        if (xml.parent and xml.parent.name == 'Folder'):
           parentName = xml.parent.find("name")
           if (parentName and parentName.string and parentName.string != ''):
             self.folder_name = parentName
    ...
    def get_folder_name(self):
        return self.folder_name.string if self.folder_name else ''
