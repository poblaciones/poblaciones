...
class SnapshotMetricVersionItemVariableModel
{
	...
	public function v2_RegenDatasetLevels($datasetId)
	{
		Profiling::BeginTimer();
		...
		$columns = $this->v2_BuildHeaders($dataset);

		$levels = $this->v2_GetDatasetLevels($dataset['dat_id']);
		foreach ($levels as $level)
		{
			$this->v2_ProcessDatasetLevel($level, $columns);
		}

		$this->v2_CreateTable($dataset, $columns);

		$this->v2_InsertValues($dataset, $columns);

		foreach ($levels as $level)
		{
			$this->v2_UpdateExtents($dataset, $level);
		}

		Profiling::EndTimer();
	}

	private function v2_CreateTable($dataset, $columns)
	{
		Profiling::BeginTimer();
	...
	private function v2_InsertValues($dataset, $columns)
	{
		Profiling::BeginTimer();
	...
	private function v2_GetDatasetLevels($datasetId)
	{
		Profiling::BeginTimer();
	...
	private function v2_ProcessDatasetLevel($metricVersionLevel, &$columns)
	{
		$variables = Variable::GetVariables($metricVersionLevel);
		foreach ($variables as $variable)
		{
			$this->v2_BuildVariableColumns($metricVersionLevel, $variable, $columns);
		}
	}
	...
	private function v2_UpdateExtents($dataset, $metricVersionLevel)
	{
		Profiling::BeginTimer();
		// Calcula para cada level
		$sql = "SELECT ST_AsText(PolygonEnvelope(LineString(
	...
	private function v2_BuildHeaders($dataset)
	{
		...
		$columns[] = ['sna_feature_id', 'bigint(11) NOT NULL', $this->v2_GetFeatureIdField($dataset)];
	...
	private function v2_BuildVariableColumns($metricVersionLevel, $variable, &$columns)
	{
		// Calcula el valor
	...
	private function v2_GetFeatureIdField($dataset)
	{
