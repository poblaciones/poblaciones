	...
	<div>
		<div v-show='hasContent && !collapsed' class='left-panel'>
			<div v-if="isFullFront">
				<ListPanel :dt='Full' ref='Full' v-if='isFullList'/>
				<InfoPanel :dt='Full' v-if='isFullInfo'/>
			</div>
			<div v-else>
				<div id="panTop" class="split split-vertical">
					<ListPanel :dt='Top' ref='Top' v-if='isTopList'/>
					<InfoPanel :dt='Top' v-if='isTopInfo'/>
				</div>
				<div id="panBottom" class="split split-vertical">
					<ListPanel :dt='Bottom' ref='Bottom' v-if='isBottomList'/>
					<InfoPanel :dt='Bottom' v-if='isBottomInfo'/>
				</div>
			</div>
		</div>
		<Button v-if='hasContent' :collapsed='collapsed' />
	</div>
</template>
...
import Button from '@/public/components/widgets/panelButton';
	...
	methods: {
		...
		updateMapTypeControl() {
			...
			if (this.collapsed) {
				...
				css.style.transform = 'translate(333px) scale(0.8)';
			}
		},
