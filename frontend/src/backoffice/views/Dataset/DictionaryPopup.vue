	...
	<div>
		<md-dialog :md-active.sync="open" class="content-sized-dialog">
			...
			<mp-confirm ref="confirmDialog"
									...
									text="Las columnas seleccionadas junto con sus datos serán eliminadas. Si deseara luego recuperar estos valores deberá volver a importar los datos al dataset."
									confirm-text="Eliminar"
									@confirm="deleteOnClick" />
			<md-dialog-content style="padding-bottom: 0px">
				<div style="position: relative">
					<JqxGrid ref="columnsGrid" :width="800" :source="dataAdapter" :columns="columns" :columnsresize="true"
					...
					<div v-if="Work.CanEdit()" style="display: inline-block; vertical-align: top;">
						<md-button @click="upOnClick()" class="md-icon-button" :disabled="upDisabled">
							<md-icon>arrow_upward</md-icon>
							<md-tooltip md-direction="left">Subir</md-tooltip>
						</md-button>
						<br />
						<md-button @click="downOnClick()" class="md-icon-button" :disabled="downDisabled">
							<md-icon>arrow_downward</md-icon>
							<md-tooltip md-direction="left">Bajar</md-tooltip>
						</md-button>
					</div>
...
export default {
	...
	methods: {
      ...
      return false;
		...
		show() {
			// El source se llena antes de abrir para que el widget jqx se cree ya
			// enlazado a los datos, igual que hace ValuesPopup con bindData.
			this.source.localdata = this.getData();
			this.source.totalrecords = this.source.localdata.length;
			this.open = true;
			this.waitForGrid(this.loadData, 60);
		},
		gridIsReady() {
			// Alcanza con que exista el componente y exponga su API. No debe
			// usarse getdatainformation() como prueba: falla mientras el grid no
			// tiene datos, con lo que la carga quedaría esperando para siempre y
			// la grilla se mostraría vacía.
			return !!(this.Grid() && typeof this.Grid().updatebounddata === 'function');
		},
		waitForGrid(callback, attempts) {
			var loc = this;
			if (this.gridIsReady()) {
				callback();
				return;
			...
			setTimeout(function () {
				loc.waitForGrid(callback, attempts - 1);
			}, 25);
		},
		dblClickModify(e) {
		...
		loadData() {
			...
			// El grid se inicializa mientras el diálogo todavía está oculto, así
			// que puede haber quedado sin dibujar: se fuerza el refresco.
			if (typeof this.Grid().refresh === 'function') {
				this.Grid().refresh();
			}
			this.hideWait();
			this.updateCount();
